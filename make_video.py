...
PROJECT_ROOT = HERE  # self-contained repo: uploader/token live next to this script
...
BED_FILE = os.path.join(ASSETS_DIR, "bed_wind.m4a")  # eerie ambience under narration
...
def expand_prompts(base, target=NUM_IMAGES):
    ...
    scene, so the narrative order is preserved (film-style coverage).
...
def build_audio_bed():
    """Generate (once, cached) an eerie wind ambience mixed under narration."""
    if os.path.exists(BED_FILE):
        return BED_FILE
    os.makedirs(ASSETS_DIR, exist_ok=True)
    log("Generating cached audio bed (one-time)...")
    run(["ffmpeg", "-y", "-f", "lavfi",
         "-i", "anoisesrc=color=pink:sample_rate=44100:duration=20",
         "-af", "highpass=f=120,lowpass=f=650,tremolo=f=0.1:d=0.5,volume=2.2",
...
def slots_filled_today():
    ...
    Counts channel-local-today rows that went out public or were scheduled, so
    catch-up runs know how many more to build.
    """
...
def slot_publish_time(idx):
    ...
    None => publish immediately (the slot's post-time has passed this run).
    """
